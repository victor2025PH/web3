            ...
            >
                ...
                >
                    ...
                    <div className="flex items-center gap-2 mb-1 border-b border-white/10 pb-1">
                        ...
                        <span className="text-[10px] font-mono font-bold tracking-wider text-zinc-300">GAME_FEED</span>
                    </div>
...
const EveBot: React.FC<{ 
    ...
    const [eyeColor, setEyeColor] = useState('#FF6B00');
    const [spinRotation, setSpinRotation] = useState(0);

    useEffect(() => {
        const colors = ['#FF6B00', '#FF8C42', '#FFA500', '#FFD700', '#FF6B35'];
        let colorIndex = 0;
        ...
        >
            ...
            >
                ...
                <motion.div 
                    ...
                    style={{ background: 'radial-gradient(circle at 50% 10%, #ffffff 0%, #fff4e6 60%, #ffe0b2 100%)' }}
                    animate={{
                ...
                <div className="relative z-20 w-[4rem] h-[5.5rem] mt-[-10px]">
                    <div 
                        ...
                        style={{ 
                            background: 'radial-gradient(circle at 30% 50%, #ffffff 0%, #fff4e6 50%, #ffe0b2 100%',
                    ...
                    >
                        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[90%] h-5 bg-gradient-to-b from-[#ffd4a3] to-transparent rounded-b-full opacity-40 blur-[1px]" />
                        <div className="absolute top-[-2px] left-1/2 -translate-x-1/2 w-[80%] h-3 bg-orange-200/50 rounded-b-full blur-[2px]" />
        ...
        >
            <motion.div
                ...
                onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    let triggerPrompt = "用户点击了机器人。你需要扮演游戏顾问，介绍红包游戏方案。";
                    const hotProducts = [
                        "1. 🎮 红包游戏核心机制",
                        "2. 💰 Web3 支付集成",
                        "3. 📈 病毒式传播策略",
                    ...
                    let aiMsg = `欢迎来到 RedEnvelope.fi！\n\n我是您的游戏顾问。我可以为您介绍：\n\n${hotProducts.join('\n')}\n\n您想了解哪个方面？`;
                    let suggestions = "了解游戏机制|查看技术架构|了解变现模式";
                    if (mode === 'idle_news') {
                        triggerPrompt = `用户对新闻 "${newsText}" 感兴趣。`;
                        aiMsg = `正在分析：${newsText}\n\n基于此趋势，我为您推荐相关解决方案。`;
                        suggestions = "了解详情|查看案例|联系专家";
                    } else if (mode === 'idle_dance') {
                         triggerPrompt = "用户在机器人跳舞时点击。";
                         aiMsg = "系统心情指数：100% ⚡\n\n趁我心情好，给您透露点内部数据。\n目前红包游戏的转化率最高。\n\n您想看实测数据吗？";
                         suggestions = "看转化数据|推荐方案|无需推荐";
                    }
