...
const parseInline = (text: string) => {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (raw.startsWith('**')) {
      elements.push(<strong key={key} className="text-purple-200 font-bold">{raw.slice(2, -2)}</strong>);
    } else if (raw.startsWith('`')) {
      elements.push(<code key={key} className="bg-zinc-800/80 text-purple-300 px-1.5 py-0.5 rounded text-[10px] font-mono border border-zinc-700/50">{raw.slice(1, -1)}</code>);
    } else if (raw.startsWith('*')) {
      elements.push(<em key={key} className="text-blue-300 not-italic">{raw.slice(1, -1)}</em>);
    }
          ...
          return (
            <div key={index} className="my-3 rounded-md bg-zinc-950 border border-zinc-800/80 overflow-hidden shadow-lg group/code">
               ...
               <div className="p-3 overflow-x-auto custom-scrollbar bg-black/50">
                  <pre className="font-mono text-xs text-purple-300/90 whitespace-pre-wrap break-all leading-relaxed">
                    {codeContent.trim()}
                  </pre>
               </div>
            </div>
        ...
        return (
          <div key={index} className="space-y-1">
            {part.split('\n').map((line, i) => {
                ...
                if (line.startsWith('### ')) return <h3 key={i} className="text-purple-400 font-bold text-sm mt-3 mb-1 tracking-wide">{parseInline(line.slice(4))}</h3>;
                ...
                if (trimmed.startsWith('- ') || trimmed.startsWith('• ') || trimmed.startsWith('* ')) {
                    return (
                        <div key={i} className="flex gap-2 ml-1 relative pl-2">
                             <span className="absolute left-0 top-2 w-1 h-1 rounded-full bg-purple-500/80 shrink-0" />
                 ...
                 if (orderedMatch) {
                   return (
                      <div key={i} className="flex gap-2 ml-1">
                          <span className="text-purple-500/80 font-mono text-xs mt-0.5 select-none">{orderedMatch[1]}.</span>
  ...
  return (
    ...
    >
        <div className={`max-w-[85%] rounded-lg p-3 relative group shadow-sm ${
          ...
          <div className={`text-[9px] mt-1 opacity-0 group-hover:opacity-100 transition-opacity ${msg.role === 'user' ? 'text-purple-500/50 text-right' : 'text-zinc-600'}`}>
      ...
      {isOpen && (
        ...
        <div className={!isFixed ? "fixed inset-0 z-[60] flex items-center justify-center p-4 pointer-events-none" : ""} >
          <motion.div
            ...
            className={`flex flex-col bg-zinc-950/95 border border-purple-500/40 rounded-xl shadow-[0_0_80px_-20px_rgba(139,92,246,0.3)] backdrop-blur-xl overflow-visible relative ${!isFixed ? 'pointer-events-auto' : ''}`}
          >
            {isFixed && !isMobileLayout && (
                <svg 
                    className="absolute w-6 h-6 text-purple-500/40 fill-zinc-950 z-50 pointer-events-none drop-shadow-[0_0_5px_rgba(139,92,246,0.5)]"
                    style={beakStyle}
                    viewBox="0 0 24 24"
                >
                    <path d="M12 0L24 24H0L12 0Z" fill="currentColor" stroke="rgb(139,92,246)" strokeWidth="1" />
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-purple-500/20 bg-zinc-900/50 rounded-t-xl shrink-0 cursor-move">
              <div className="flex items-center gap-3">
                <div className="relative w-8 h-8 flex items-center justify-center">
                    <div className="absolute inset-0 border border-purple-500/50 rounded-full animate-[spin_4s_linear_infinite]" />
                    <Bot className="absolute w-4 h-4 text-purple-500 z-0" />
                </div>
                <div>
                   <h3 className="text-xs font-mono font-bold text-purple-400 tracking-widest uppercase flex items-center gap-2">
                       TON Mini App <span className="text-zinc-500 text-[10px]">// DEV</span>
                   </h3>
                   <div className="flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>
                      <span className="text-[10px] text-zinc-500 font-mono">DEV CONNECTION</span>
                   </div>
                </div>
              </div>
            ...
            >
              ...
              {isTyping && (
                ...
                >
                   <div className="flex items-center gap-3 px-3 py-2 bg-zinc-900/50 border-l-2 border-purple-500 rounded-r-md">
            ...
            <AnimatePresence>
                {showSuggestions && filteredSuggestions.length > 0 && (
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 10 }}
                        className="absolute bottom-[60px] left-4 right-4 bg-zinc-900/95 border border-purple-500/30 rounded-lg shadow-xl backdrop-blur-md overflow-hidden z-20"
                    >
                                ...
                                <button
                                    ...
                                    className={`w-full flex items-center justify-between px-3 py-2 rounded border transition-all group text-left ${
                                        idx === activeSuggestionIndex 
                                            ...
                                            : 'hover:bg-purple-500/10 hover:border-purple-500/20 border-transparent'
                                    }`}
                                >
                                    <div className="flex items-center gap-2">
                                        {cmd.startsWith('/') && <Terminal className="w-3 h-3 text-purple-500/50" />}
                                        <span className={`font-mono text-xs font-bold ${idx === activeSuggestionIndex ? 'text-purple-200' : 'text-purple-400'}`}>
                                            {cmd}
                                        </span>
                                    </div>
                                    <ChevronRight className={`w-3 h-3 transition-opacity ${
                                        idx === activeSuggestionIndex ? 'opacity-100 text-purple-400' : 'opacity-0 group-hover:opacity-100 text-zinc-600'
                                    }`} />
                                </button>
                            ))}
                        </div>
            ...
            <div className="p-3 bg-zinc-900/80 border-t border-purple-500/20 rounded-b-xl shrink-0 relative z-30">
               <form onSubmit={handleSend} className="relative flex items-center gap-2">
                  <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-1">
                    <Activity className="w-4 h-4 text-purple-500/50" />
                    {input === '' && <span className="w-1.5 h-4 bg-purple-500/50 animate-pulse ml-0.5" />}
                  </div>
                  <input
                    ref={inputRef}
                    type="text"
                    ...
                    placeholder="请输入指令..."
                    className="w-full bg-black/50 border border-white/10 rounded-md py-2.5 pl-10 pr-10 text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/20 font-mono text-sm transition-all"
                    autoFocus
                    autoComplete="off"
                  />
                  <button 
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-purple-500/10 rounded hover:bg-purple-500/20 text-purple-500 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                  >
